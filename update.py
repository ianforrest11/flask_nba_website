...
for i in range(number_of_rows):
    ...
    # field goal %
    try:
        fg_pct = round(float(fgm/fga),3)
    except:
        fg_pct = round(float(0), 3)
    print('fg_pct:', fg_pct)
    ...
    # 3pt field goal %
    try:
        fg3_pct = round(float(fg3m/fg3a),3)
    except:
        fg3_pct = round(float(0), 3)
    ...
    # free throw %
    try:
        ft_pct = round(float(ftm/fta),3)
    except:
        ft_pct = round(float(0), 3)
    print('ft_pct:', ft_pct)
